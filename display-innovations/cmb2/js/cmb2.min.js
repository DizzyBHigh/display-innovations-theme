window.CMB2 = window.CMB2 || {}, function (window, document, $, cmb, undefined) {
    ...
    cmb.metabox = function () {
        ...
        cmb.initPickers($metabox.find('input[type="text"].cmb2-timepicker'), $metabox.find('input[type="text"].cmb2-datepicker'), $metabox.find('input[type="text"].cmb2-colorpicker')), $('<p><span class="button cmb-multicheck-toggle">' + l10n.strings.check_toggle + "</span></p>").insertBefore(".cmb2-checkbox-list:not(.no-select-all)"), cmb.makeListSortable(), $metabox.on("change", ".cmb2_upload_file", function () {
        ...
        }).on("click", ".cmb-shift-rows", cmb.shiftRows).on("cmb2_add_row", cmb.emptyValue), setTimeout(cmb.resizeoEmbeds, 500), $(window).on("resize", cmb.resizeoEmbeds), cmb.trigger("cmb_init")
    }, cmb.resetTitlesAndIterator = function (evt) {
        evt.group && $(".cmb-repeatable-group").each(function () {
            var $table = $(this);
        ...
        if (wp) {
            var media = cmb.media;
            media.field = formfield, media.$field = $id(media.field), media.fieldData = media.$field.data(), media.previewSize = media.fieldData.previewsize, media.fieldName = media.$field.attr("name"), media.isList = isList;
            ...
            media.frames[media.field] = wp.media({
            ...
            }), cmb.trigger("cmb_media_modal_init", media), cmb.mediaHandlers.list = function (selection, returnIt) {
                ...
                return $(attachment).each(function () {
                    if (this.type && "image" === this.type) {
                        ...
                        uploadStatus = '<li class="img-status cmb2-media-item"><img width="' + width + '" height="' + height + '" src="' + this.url + '" class="attachment-' + width + "px" + height + 'px" alt="' + this.filename + '"><p><a href="#" class="cmb2-remove-file-button" rel="' + media.field + "[" + this.id + ']">' + l10n.strings.remove_image + '</a></p><input type="hidden" id="filelist-' + this.id + '" data-id="' + this.id + '" name="' + media.fieldName + "[" + this.id + ']" value="' + this.url + '"></li>'
                    } else uploadStatus = '<li class="file-status cmb2-media-item"><span>' + l10n.strings.file + " <strong>" + this.filename + '</strong></span>&nbsp;&nbsp; (<a href="' + this.url + '" target="_blank" rel="external">' + l10n.strings.download + '</a> / <a href="#" class="cmb2-remove-file-button" rel="' + media.field + "[" + this.id + ']">' + l10n.strings.remove_file + '</a>)<input type="hidden" id="filelist-' + this.id + '" data-id="' + this.id + '" name="' + media.fieldName + "[" + this.id + ']" value="' + this.url + '"></li>';
                ...
                } else uploadStatus = '<div class="file-status cmb2-media-item"><span>' + l10n.strings.file + " <strong>" + attachment.filename + '</strong></span>&nbsp;&nbsp; (<a href="' + attachment.url + '" target="_blank" rel="external">' + l10n.strings.download + '</a> / <a href="#" class="cmb2-remove-file-button" rel="' + media.field + '">' + l10n.strings.remove_file + "</a>)</div>";
                ...
                cmb.attach_id && isList ? $('[data-id="' + cmb.attach_id + '"]').parents("li").replaceWith(cmb.mediaHandlers.list(selection, !0)) : cmb.mediaHandlers[type](selection), cmb.trigger("cmb_media_modal_select", selection, media)
            }, cmb.mediaHandlers.openModal = function () {
                var attach, selection = media.frames[media.field].state().get("selection");
        ...
        evt.preventDefault();
        var $this = $(this);
        return $this.is(".cmb-attach-list .cmb2-remove-file-button") ? ($this.parents(".cmb2-media-item").remove(), !1) : (cmb.media.field = $this.attr("rel"), cmb.metabox().find("input#" + cmb.media.field).val(""), cmb.metabox().find("input#" + cmb.media.field + "_id").val(""), $this.parents(".cmb2-media-status").html(""), !1)
        ...
        return $elements.filter(":checked").prop("checked", !1), $elements.filter(":selected").prop("selected", !1), $row.find("h3.cmb-group-title").length && $row.find("h3.cmb-group-title").text($row.data("title").replace("{#}", cmb.idNumber + 1)), $elements.each(function () {
            cmb.elReplacements($(this), prevNum)
        }), cmb
    }, cmb.elReplacements = function ($newInput, prevNum) {
        var newID, oldID, oldFor = $newInput.attr("for"), oldVal = $newInput.val(), type = $newInput.prop("type"), checkable = "radio" === type || "checkbox" === type ? oldVal : !1, attrs = {};
        ...
        return (undefined !== typeof oldVal && oldVal || checkable) && (attrs.value = checkable ? checkable : ""), "TEXTAREA" === $newInput.prop("tagName") && $newInput.html(""), checkable && $newInput.removeAttr("checked"), $newInput.removeClass("hasDatepicker").attr(attrs).val(checkable ? checkable : ""), $newInput
        ...
        if ("undefined" != typeof name) {
            var prevNum = parseInt($this.parents(".cmb-repeatable-grouping").data("iterator"), 10), newNum = prevNum - 1, $newName = name.replace("[" + prevNum + "]", "[" + newNum + "]");
        ...
        var $table = $id($this.data("selector")), $oldRow = $table.find(".cmb-repeatable-grouping").last(), prevNum = parseInt($oldRow.data("iterator"), 10);
        ...
        $oldRow.after($newRow), cmb.afterRowInsert($newRow), $table.find(".cmb-repeatable-grouping").length <= 1 ? $table.find(".cmb-remove-group-row").prop("disabled", !0) : $table.find(".cmb-remove-group-row").prop("disabled", !1), cmb.triggerElement($table, {
            type: "cmb2_add_row",
            group: !0
        }, $newRow)
    }, cmb.addAjaxRow = function (evt) {
        evt.preventDefault();
        var $this = $(this), $table = $id($this.data("selector")), $emptyrow = $table.find(".empty-row"), prevNum = parseInt($emptyrow.find("[data-iterator]").data("iterator"), 10);
        cmb.idNumber = parseInt(prevNum, 10) + 1;
        ...
        cmb.newRowHousekeeping($row).cleanRow($row, prevNum), $emptyrow.removeClass("empty-row hidden").addClass("cmb-repeat-row"), $emptyrow.after($row), cmb.afterRowInsert($row), cmb.triggerElement($table, {
            type: "cmb2_add_row",
            group: !1
        }, $row), $table.find(".cmb-remove-row-button").removeClass("button-disabled")
    }, cmb.removeGroupRow = function (evt) {
        evt.preventDefault();
        var $this = $(this), $table = $id($this.data("selector")), $parent = $this.parents(".cmb-repeatable-grouping"), number = $table.find(".cmb-repeatable-grouping").length;
        2 > number || (cmb.triggerElement($table, "cmb2_remove_group_row_start", $this), $parent.nextAll(".cmb-repeatable-grouping").find(cmb.repeatEls).each(cmb.updateNameAttr), $parent.remove(), 2 >= number ? $table.find(".cmb-remove-group-row").prop("disabled", !0) : $table.find(".cmb-remove-group-row").prop("disabled", !1), cmb.triggerElement($table, {
        ...
        if (!$this.hasClass("button-disabled")) {
            var $parent = $this.parents(".cmb-row"), $table = $this.parents(".cmb-repeat-table"), number = $table.find(".cmb-row").length;
            number > 2 ? ($parent.hasClass("empty-row") && $parent.prev().addClass("empty-row").removeClass("cmb-repeat-row"), $this.parents(".cmb-repeat-table .cmb-row").remove(), 3 === number && $table.find(".cmb-remove-row-button").addClass("button-disabled"), cmb.triggerElement($table, {
                type: "cmb2_remove_row",
                group: !1
        ...
        evt.preventDefault();
        var $this = $(this), $from = $this.parents(".cmb-repeatable-grouping"), $goto = $this.hasClass("move-up") ? $from.prev(".cmb-repeatable-grouping") : $from.next(".cmb-repeatable-grouping");
        ...
        $selector.length && $selector[method]("destroy").each(function () {
            var $this = $(this), fieldOpts = $this.data(method) || {}, options = $.extend({}, cmb.defaults[defaultKey], fieldOpts);
        ...
        return options.beforeShow = function (input, inst) {
            ...
            $id("ui-datepicker-div").removeClass("cmb2-element").hide(), "function" == typeof existing.onClose && existing.onClose(dateText, inst)
        }, options
        ...
        cmb.metabox().each(function () {
            var $this = $(this), $tableWrap = $this.parents(".inside"), isSide = $this.parents(".inner-sidebar").length || $this.parents("#side-sortables").length, isSmall = isSide, isSmallest = !1;
            ...
            var $embeds = $this.find(".cmb-type-oembed .embed-status"), $children = $embeds.children().not(".cmb2-remove-wrapper");
    ...
    function initOptions(options) {
        var prop, newSettings, newQTS, nameRegex = new RegExp("cmb2_n_" + options.groupid + options.fieldid, "g"), idRegex = new RegExp("cmb2_i_" + options.groupid + options.fieldid, "g");
    ...
    wysiwyg.initAll = function () {
        ...
        }), !0 === initiated && ("undefined" != typeof window.QTags && window.QTags._buttonsInit(), $(document).on("cmb2_add_row", wysiwyg.addRow).on("cmb2_remove_group_row_start", wysiwyg.destroyRowEditors).on("cmb2_shift_rows_start", wysiwyg.shiftStart).on("cmb2_shift_rows_complete", wysiwyg.shiftComplete))
        ...
        $row.find(".cmb2-wysiwyg-inner-wrap").each(function () {
            $toReplace = $(this), data = $toReplace.data(), data.iterator = $row.data("iterator"), data.fieldid = data.id, data.id = data.groupid + "_" + data.iterator + "_" + data.fieldid, data.name = data.groupid + "[" + data.iterator + "][" + data.fieldid + "]", data.value = $toReplace.find(".wp-editor-area").length ? $toReplace.find(".wp-editor-area").val() : "", 0 === toBeDestroyed.length ? wysiwyg.init($toReplace, data) : (toBeInitialized.push([$toReplace, data]), window.setTimeout(delayedInit, 100))
        })
    }, wysiwyg.init = function ($toReplace, data, buttonsInit) {
        return data.groupid ? ($.extend(data, getGroupData(data)), initOptions(data), $toReplace.replaceWith(data.template(data)), window.tinyMCE.init(tinyMCEPreInit.mceInit[data.id]), "function" == typeof window.quicktags && window.quicktags(tinyMCEPreInit.qtInit[data.id]), $(document.getElementById(data.id)).parents(".wp-editor-wrap").removeClass("html-active").addClass("tmce-active"), void(!1 !== buttonsInit && "undefined" != typeof window.QTags && window.QTags._buttonsInit())) : !1
    }, wysiwyg.destroy = function (id) {
        var editor = tinyMCE.get(id);
        null !== editor && "undefined" != typeof editor ? (editor.destroy(), "undefined" == typeof tinyMCEPreInit.mceInit[id] && delete tinyMCEPreInit.mceInit[id], "undefined" == typeof tinyMCEPreInit.qtInit[id] && delete tinyMCEPreInit.qtInit[id]) : -1 === toBeDestroyed.indexOf(id) && (toBeDestroyed.push(id), window.setTimeout(delayedDestroy, 100))
